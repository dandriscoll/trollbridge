...
- The **proxy host** runs `trollbridge run`. It owns the CA private
  ...
  and the LLM API key file (`/etc/trollbridge/llm.key`).
  Privileged operations: `ca init`, `ca client-cert`, writing the
  LLM key, restarting the daemon.
- The **consumer host** is any machine running apps whose HTTP
  egress goes through the proxy. It owns a copy of the CA's *public*
  cert in its system trust store.
  Operation: `ca install` (and only that).
